...
namespace OpenTK_PathTracer.GUI
{
    ...
    public class ImGuiController : IDisposable
    {
        ...
        {
            vbo = new BufferObject(BufferTarget.ArrayBuffer, 10000, BufferUsageHint.DynamicDraw);
            ebo = new BufferObject(BufferTarget.ElementArrayBuffer, 2000, BufferUsageHint.DynamicDraw);
            ...
            vao.SetAttribPointer(0, 2, VertexAttribPointerType.Float, 0 * sizeof(float));
            vao.SetAttribPointer(1, 2, VertexAttribPointerType.Float, 2 * sizeof(float));
            vao.SetAttribPointer(2, 4, VertexAttribPointerType.UnsignedByte, 4 * sizeof(float), true);
        }
        ...
        private void RenderImDrawData(ImDrawDataPtr drawData)
        {
            ...
            for (int i = 0; i < drawData.CmdListsCount; i++)
            {
                ...
                for (int cmd_i = 0; cmd_i < cmd_list.CmdBuffer.Size; cmd_i++)
                {
                    ...
                    {
                        Texture.AttachToUnit(TextureUnit.Texture0, TextureTarget.Texture2D, (int)pcmd.TextureId);
